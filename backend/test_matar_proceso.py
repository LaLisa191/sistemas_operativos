"""
Script de prueba para verificar el endpoint DELETE /procesos/{pid}
Ejecutar: python test_matar_proceso.py
"""
...
def test_matar_proceso(pid):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"⚠️ Acceso denegado (403): {response.json().get('detail', 'Sin permisos')}")
            print("   Ejecuta el backend como administrador")
            return False
        elif response.status_code == 404:
            print(f"⚠️ Proceso no encontrado (404)")
            return False
        else:
...
def main():
    ...
    print(f"\n3️⃣ Verificando que el proceso {pid_prueba} existe...")
